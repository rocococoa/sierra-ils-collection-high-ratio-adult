/* Retrieves a list of adult print and all media titles that have over a 4:1 hold ratio 
   OR have holds but 0 holdable items (excludes On Order titles).
   created AGW 4/2026
   */
...
item_stats AS (
    SELECT 
        ...
        COUNT(i.id) FILTER (
            ...
            AND i.location_code NOT IN ('mfolb', 'mfold', 'bfol','bfolx')
        ) AS item_totals,
        COUNT(i.id) FILTER (
            WHERE i.item_status_code NOT IN ('$', 'n', 'w', 'm')
            AND i.location_code IN ('mfolb', 'mfold', 'bfol','bfolx')
...
WHERE
    (
        (hc.hold_count::numeric / NULLIF(its.item_totals, 0) >= 4)
        OR 
        (COALESCE(its.item_totals, 0) = 0 AND hc.hold_count >= 1)
    )
	AND brp.material_code IN ('f', 'd', 'l', 'e', '8', 'c', 'i', 'o', 'g', 'm', 'k', 'j')
	AND brp.best_title NOT LIKE '%[videorecording]%' 
	AND brp.best_title NOT LIKE '%[sound recording]%'
ORDER BY
    "Call#",
    brp.best_author,
	"Ratio" DESC
